...
public class EmployeeService {
	...
	//Find out employees with highest count.
	public void deptWithHighestCount()
	{
		Map<String, List<Employee>> map = EmployeeRepository.getEmployees()
				.stream().filter(emp -> (emp.getDepartment() == null)).collect(Collectors.groupingBy((employee)->employee.getDepartment().getDepartmentName()));
		int max = 0;
		String department ="";
	...
	public void sortByAttribute()
	{
		...
		List<Employee> sortByDeptID=employeeList.stream()
				.filter((e) -> e.getDepartment()!=null)
				.sorted(Comparator.comparing((emp) -> emp.getDepartment().getDepartmentId()))
				.collect(Collectors.toList());
		 sortByDeptID.forEach(System.out::println);
